...
wide_to_long <- function(df, baseline_variables, wide_variables, visit_times,
                         outcome_times) {
  # this also need outcome data, as we don't want rows after event (I think)
...
#' @export
add_lag_terms <- function(df, var, lag, fill = 0) {
  for (l in lag) {
...
#' @export
ip_score_long <- function(data_long, outcome, time_horizon, treatment_formula,
                          treatment_of_interest, n_visits, outcome, time_horizon) {
  ...
  outcome <- extract_outcome(data_long, substitute(outcome), time_horizon)
